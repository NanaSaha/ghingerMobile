import { Component, ViewChild } from "@angular/core";
import {
  IonicPage,
  NavController,
  NavParams,
  ViewController,
} from "ionic-angular";
import { FormBuilder, FormGroup, Validators } from "@angular/forms";
import {
  ToastController,
  LoadingController,
  AlertController,
  ModalController,
} from "ionic-angular";
import { DataProvider } from "../../providers/data/data";
// import { Search2Page } from '../search2/search2';
// import { SearchPage } from '../search/search';
import { CompleteTestService } from "../../providers/complete-test-service/complete-test-service";
// import { Keyboard } from '@ionic-native/keyboard';
import "rxjs/add/operator/map";
import { Storage } from "@ionic/storage";
import { ConfirmmomopaymentPage } from "../confirmmomopayment/confirmmomopayment";
import { InAppBrowser } from "@ionic-native/in-app-browser";
import { MenuPage } from "../menu/menu";

@Component({
  selector: "page-momopayment",
  templateUrl: "momopayment.html",
})
export class MomopaymentPage {
  public paymentForm: any;
  public paymentCardForm: any;
  currentmedappointmentdetail: any;
  currentmedappointmentdetaildata = {
    id: 0,
    location: "",
    serviceprovider: "",
    requestcategory: "",
    beneficiary: "",
    requesturgency: "",
    proposeddateandtime: "",
    complaint: "",
    prevmedicalhistory: "",
    allergies: "",
    source: "",
    confirmstatus: "",
    medications: "",
    created_at: "",
    beneficiary_phone_number: "",
    beneficiary_age: "beneficiary_age",
    beneficiary_gender: "",
    confirmed_date: "",
  };
  params: any = [];
  newparams: any;
  appointment_id: any;
  jsonBody: any;
  body: any;
  retrieve1: string;
  body1: any;
  jsonBody1: any;
  medication_appoint_history: any;
  billdetails: any;
  my_params: any;
  bill_details: any;
  new_bill_d: any;
  item_total = 0.0;
  final_item_total: string;
  patient_billing_infos_id: string;
  paid_status: string;
  patient_name: string;
  appointment_type_title: string;
  bill_appointment_title: string;
  bill_amount: string;
  appointment_title: string;
  bill_amount_words: string;
  billing_info_id: string;
  sender_network_long: string;

  from_login;
  from_params;
  amount;
  payment_type;
  Val: any;
  qp = [];
  redirect;
  subscription_id;
  login_list;
  user_id;

  constructor(
    public iab: InAppBrowser,
    public navCtrl: NavController,
    public navParams: NavParams,
    public _form: FormBuilder,
    public completeTestService: CompleteTestService,
    public data: DataProvider,
    public loadingCtrl: LoadingController,
    public alertCtrl: AlertController,
    public modalCtrl: ModalController,
    public viewCtrl: ViewController,
    public storage: Storage
  ) {
    this.from_login = this.navParams.get("value");
    this.from_params = this.navParams.get("params");
    this.payment_type = this.navParams.get("payment_type");
    this.subscription_id = this.navParams.get("subscription_id");
    this.amount = this.from_params;

    console.log("VALUE IN Login IS" + this.from_login);
    this.login_list = JSON.stringify(this.from_login);
    this.jsonBody = JSON.parse(this.login_list);
    this.user_id = this.jsonBody[0].id;
    console.log("USER ID " + this.user_id);

    console.log("AMOUNT FROM PARAMS " + this.amount);
    console.log("PAYMENT TYPE " + this.payment_type);
    console.log("subscription_id " + this.subscription_id);

    this.storage.get("patient_name").then((patient_name) => {
      this.patient_name = patient_name;
      console.log("USER NAME " + this.patient_name);
    });

    this.storage
      .get("bill_appointment_title")
      .then((bill_appointment_title) => {
        this.bill_appointment_title = bill_appointment_title;
      });

    this.storage.get("bill_amount").then((bill_amount) => {
      this.bill_amount = bill_amount;
    });

    this.storage.get("appointment_title").then((appointment_title) => {
      this.appointment_title = appointment_title;
    });

    this.storage.get("bill_amount_words").then((bill_amount_words) => {
      this.bill_amount_words = bill_amount_words;
    });

    this.storage.get("billing_info_id").then((billing_info_id) => {
      this.billing_info_id = billing_info_id;
    });

    this.paymentForm = this._form.group({
      patient_name: ["", Validators.compose([Validators.required])],
      customer_number: [""],
      nw: ["", Validators.compose([Validators.required])],
      // voucher_code: [
      //   "",
      //   ExtraValidators.conditional(
      //     (group) => group.controls.nw.value == "VOD",
      //     Validators.compose([
      //       Validators.minLength(6),
      //       Validators.maxLength(6),
      //       Validators.pattern("^[0-9]+$"),
      //       Validators.required,
      //     ])
      //   ),
      // ],
      amount: ["", Validators.compose([Validators.required])],
    });

    this.paymentCardForm = this._form.group({
      patient_name: ["", Validators.compose([Validators.required])],

      amount: ["", Validators.compose([Validators.required])],
    });
  }

  ionViewDidLoad() {
    console.log("ionViewDidLoad Medication appointment details Page");
  }

  makePayment() {
    switch (this.paymentForm.value.nw) {
      case "MTN":
        this.sender_network_long = "MTN";
        break;
      case "TIG":
        this.sender_network_long = "TIG";
        break;
      case "VOD":
        this.sender_network_long = "VOD";
        break;
      case "AIR":
        this.sender_network_long = "AIR";
        break;

      default:
        break;
    }

    this.navigate_to_momo_confirm();
  }

  navigate_to_momo_confirm() {
    console.log("moving to confirmmomopayment");

    this.storage.set(
      "bill_phone_number",
      this.paymentForm.value.customer_number
    );
    this.storage.set("bill_sender_network", this.paymentForm.value.nw);
    this.storage.set("bill_voucher_code", this.paymentForm.value.voucher_code);
    this.storage.set("bill_sender_network_long", this.sender_network_long);

    this.navCtrl.push(ConfirmmomopaymentPage, {
      customer_number: this.paymentForm.value.customer_number,
      nw: this.paymentForm.value.nw,
      amount: this.paymentForm.value.amount,
      voucher_code: this.paymentForm.value.voucher_code,
      subscription_id: this.subscription_id,
      user_id: this.user_id,
    });
  }

  card_payment() {
    let loading = this.loadingCtrl.create({
      content:
        "Redirecting you to a secured payment page to complete your card payment...",
      duration: 10000,
    });

    loading.present();

    setTimeout(() => {
      this.params = {
        subscription_id: this.subscription_id,
        nickname: this.patient_name,
        amount: this.amount,
        user_id: this.user_id,
      };

      this.Val = JSON.stringify(this.params);
      this.jsonBody = JSON.parse(this.Val);
      console.log("THIS IS the book appointment values " + this.Val);

      this.data.make_payment_card(this.jsonBody).then((result) => {
        console.log("make_payment_url RESULT = ");
        console.log(result);
        this.body = result["_body"];
        var json_parsed = JSON.parse(this.body);
        var re_parsed = JSON.parse(json_parsed);
        console.log("SEE BODY" + json_parsed);
        console.log("RESP" + re_parsed);

        this.qp = JSON.parse(re_parsed);
        console.log(this.qp);
        console.log(this.qp["redirect_url"]);
        this.redirect = this.qp["redirect_url"];

        const browser = this.iab.create(this.redirect);
        setTimeout(() => {
          loading.dismiss();
        }, 10000);

        let alert = this.alertCtrl.create({
          title: "Ghinger Health Care",
          subTitle: "Go To Menu",
          buttons: [
            {
              text: "OK",
              handler: () => {
                this.navCtrl.setRoot(MenuPage);
              },
            },
          ],
        });
        alert.present();
      });
      loading.dismiss();
    });
  }

  //     if (this.body) {
  //       if (this.body.resp_code) {
  //         if (this.body.resp_code == "000") {
  //           let alert = this.alertCtrl.create({
  //             title: "Ghinger Health Care",
  //             subTitle: this.body.resp_desc,
  //             buttons: [
  //               {
  //                 text: "OK",
  //                 handler: () => {
  //                   //this.navCtrl.setRoot(MenuPage);
  //                 },
  //               },
  //             ],
  //           });
  //           alert.present();
  //         } else {
  //           this.showalertmessage(this.body.resp_desc);
  //         }
  //       }
  //     }
  //   },
  //   (err) => {
  //     loading.dismiss();
  //     this.showalertmessage(
  //       "Financial Platform having some issues currently and will be resolved quickly. Kindly try again later"
  //     );
  //     console.log(err);
  //   }
  // );
  // });

  closeModal() {
    this.viewCtrl.dismiss();
  }

  showalertmessage(mainmsg) {
    let alert = this.alertCtrl.create({
      title: "Ghinger Health Care",
      subTitle: mainmsg,
      buttons: ["OK"],
    });
    alert.present();
  }
}

export class ExtraValidators {
  static conditional(conditional, validator) {
    return function (control) {
      revalidateOnChanges(control);

      if (control && control._parent) {
        if (conditional(control._parent)) {
          return validator(control);
        }
      }
    };
  }
}

function revalidateOnChanges(control): void {
  if (control && control._parent && !control._revalidateOnChanges) {
    control._revalidateOnChanges = true;
    control._parent.valueChanges
      .distinctUntilChanged((a, b) => {
        // These will always be plain objects coming from the form, do a simple comparison
        if ((a && !b) || (!a && b)) {
          return false;
        } else if (a && b && Object.keys(a).length !== Object.keys(b).length) {
          return false;
        } else if (a && b) {
          for (let i in a) {
            if (a[i] !== b[i]) {
              return false;
            }
          }
        }
        return true;
      })
      .subscribe(() => {
        control.updateValueAndValidity();
      });

    control.updateValueAndValidity();
  }
  return;
}
