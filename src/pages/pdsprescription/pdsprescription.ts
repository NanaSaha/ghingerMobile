...
@Component({
  selector: "pdsprescription",
  templateUrl: "pdsprescription.html",
})
export class Pdsprescription {
  ...
  doctor_name: any;
  appointment_id: any;
  doctor_mobile_number: string;
  doctor_licence_number: string;
  ...
  constructor(
    ...
    this.appointForm = this._form.group({
      prescription_name: ["", Validators.compose([Validators.required])],
      drugs_quantity: ["", Validators.compose([Validators.required])],
      drugs_dosage: ["", Validators.compose([Validators.required])],
      drugs_frequency: ["", Validators.compose([Validators.required])],
      number_of_days: ["", Validators.compose([Validators.required])],
    });
    ...
    this.storage.get("doctor_mobile_number").then((doctor_mobile_number) => {
      this.doctor_mobile_number = doctor_mobile_number;
      console.log(
        " Placeorderforpatient doctor_mobile_number = " + doctor_mobile_number
      );
    });

    this.storage.get("doctor_licence_number").then((doctor_licence_number) => {
      this.doctor_licence_number = doctor_licence_number;
      console.log(
        " PDS Prescription doctor_licence_number = " + doctor_licence_number
      );
    });
    ...
    this.patient_name = this.navParams.get("full_name");
    this.email = this.navParams.get("email");
    this.patient_age = this.navParams.get("beneficiary_age");
  ...
  attend_to() {
    ...
    this.data.attend_to_pds_prescription(this.params).then(
      (result) => {
        console.log("THIS IS THE RESULT" + result);
        var jsonBody = result["_body"];
        console.log(jsonBody);
  ...
  getappointmenttypes() {
    this.data.get_appointment_types1().then(
      (result) => {
  ...
  getserviceproviders() {
    this.data.get_service_providers().then(
      (result) => {
        console.log("RESULTS IS " + result);
        var body = result["_body"];
        console.log("result body = " + JSON.stringify(result["_body"]));
  ...
  createPdf() {
    ...
    var docDefinition = {
      content: [
        ...
        {
          text: "Prescription: " + this.appointForm.value.prescription_name,
          style: "body",
        },

        {
          text: "Dosage: " + this.appointForm.value.drugs_dosage,
          style: "body",
        },
        {
          text: "Frequency per day: " + this.appointForm.value.drugs_frequency,
          style: "body",
        },
        {
          text: "Number of days: " + this.appointForm.value.number_of_days,
          style: "body",
        },
        ...
        { text: this.doctor_licence_number + " ", style: "signature" },
        { text: this.doctor_mobile_number + " ", style: "signature" },
      ],
      styles: {
        header: {
          fontSize: 28,
  ...
  downloadPdf() {
    if (this.platform.is("cordova")) {
      this.pdfObj.getBuffer((buffer) => {
        ...
        this.file
          .writeFile(this.file.dataDirectory, "prescription.pdf", blob, {
            replace: true,
          })
          .then((fileEntry) => {
  ...
  emailPdf() {
    ...
    let confirm = this.alertCtrl.create({
      title: "Email File?",
      message:
        "Are you sure you want to send this prescription file to " +
        this.patient_name +
        " via Email? You can click No and then click on the Create Pdf button  in order to preview the file first.",
      buttons: [
        {
          text: "No",
          handler: () => {
  ...
  email_func() {
    ...
    let email = {
      ...
      body:
        "Hello " +
        this.patient_name +
        ", you can find below a copy of your prescriptions. Kindly adhere to the instructions given. Have a lovely day. Dr. " +
        this.doctor_name,
      isHtml: true,
    };
